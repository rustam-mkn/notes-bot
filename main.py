...
- Для obsidian_md верни только исправленное тело заметки без заголовка.
"""
...
def format_obsidian_note(text: str) -> str:
    stamp = datetime.now().strftime('%d.%m.%Y')
    cleaned = text.strip()
    if not cleaned:
        return f"___\n___\n{stamp}"
...
def enforce_title_and_date(note: NormalizedNote, raw_text: str) -> NormalizedNote:
    ...
    return NormalizedNote(
        title=title,
        obsidian_md=format_obsidian_note(normalized_body) + '\n',
...
def call_mistral(raw_text: str) -> NormalizedNote:
    ...
    obsidian_md = format_obsidian_note((data.get('obsidian_md') or apple_notes_text).strip()) + '\n'
    return NormalizedNote(
        title=title,
